...
export function initializeDebugEventIndexDisplay(idx: number) {
    return `
const debugInfoDiv = document.createElement("div");
debugInfoDiv.setAttribute("id", "screenrecording-debug-event-idx");
debugInfoDiv.innerText = "Event Index: " + ${idx};

debugInfoDiv.style.position = "fixed";
debugInfoDiv.style.pointerEvents = "none";
debugInfoDiv.style.zIndex = "100000";

debugInfoDiv.style.left = "0";
debugInfoDiv.style.top = "0";
debugInfoDiv.style.backgroundColor = "white";
debugInfoDiv.style.padding = "10px";
debugInfoDiv.style.border = "1px solid black";
debugInfoDiv.style.fontSize = "28px";

document.body.appendChild(debugInfoDiv);
`;
}

export function updateDebugEventIndexDisplay(idx: number) {
    return `
document.getElementById("screenrecording-debug-event-idx").innerText = "Event Index: " + ${idx};
`;
}
